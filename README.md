...
Encrypted ZFS mirror on the NAS, using a key derived from a
  ...
  Split encryption roots: Gitea stays unlocked all day, but your
  laptop backups are cryptographically sealed for ~23.9 hours a day
  and only unlocked for the duration of the nightly sync.
  ...
  Nightly "bracketed" rsync backup of the laptop's home directory 
  at 03:00, with sensible excludes for caches, container layers, and Steam.
  Auto-update + garbage collect + optimise + reboot at 03:30–04:00
  on both machines.
  Automatic gitea unlock at 04:05 from the laptop, so your git server
  comes back up after the nightly NAS reboot without manual intervention.
  A manual `unlock-nas` helper command is also provided for unscheduled reboots.
...
Hardware assumptions
  ...
  A NAS-style machine with at least three drives:
    One small drive for NixOS itself
...
5. Deploy the NAS config: copy `nas-configuration.nix` to the NAS's
...
6. Run `nas-setup.sh` on the NAS, providing the encryption key and
   ...
   laptop terminal, where copy-paste works), then just run the script.
   It will ask you for those drive locations after the 30 second wait.

8. Restart gitea on the NAS: `sudo systemctl restart gitea`. It
   should now be reachable at `http://YOUR_NAS_IP:3000`.

9. Edit `laptop-configuration.nix` to fill in your username, the
...
Daily operation
    ...
    03:00 — Laptop unlocks the backup dataset, rsyncs home dir to NAS, 
    and immediately re-locks the dataset.
    ...
    04:05 — Laptop unlocks the gitea dataset on the NAS and restarts 
    gitea. (The backup dataset remains sealed until the next 03:00 sync).

Gitea and sanoid become available again automatically once 04:05 fires.
If the NAS reboots unexpectedly during the day, run unlock-nas on 
the laptop to bring gitea back up.
...
The mount count should be 1 during the day (just gitea, since backups 
are sealed), or 3 immediately after the 03:00 backup runs.
Recovery
If the laptop dies, the NAS continues working but won't auto-unlock
after future reboots. Use recover.sh from any Linux machine to
unlock manually using your memorized passphrase.
If both die, move the two data disks to any machine with ZFS support
and run recover.sh there. It walks through:

    Importing the pool (with -f since it remembers a different host)
    Re-deriving the encryption key from your passphrase
    Unlocking both the gitea and backup encryption roots
    Pulling your data and authorizing the new machine's SSH key
    Re-locking the backup dataset to restore the nightly security posture

Keep a copy of recover.sh somewhere outside the laptop: a USB
...
Security model
    ...
    Threat: someone compromises the NAS OS while it's running. In a 
    standard setup, an attacker with root access can read all data on the 
    mounted disks. With the "split encryption roots" design, your laptop 
    backups are cryptographically sealed and completely inaccessible to 
    the NAS OS for ~23.9 hours a day.
...
Customization notes
    ...
    the configs. They use systemd.time(7)
