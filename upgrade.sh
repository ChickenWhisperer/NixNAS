...
# The NixOS gitea module's pre-start script copies a default app.ini
# into /tank/gitea/custom/conf/ but doesn't create the directory tree
# first. On a fresh (empty) dataset this causes the pre-start to fail
# with "No such file or directory" and gitea hits the start-limit.
# Create the skeleton and hand ownership to the gitea user (which the
# NixOS config has already created via services.gitea.enable = true).
mkdir -p /tank/gitea/custom/conf
chown -R gitea:gitea /tank/gitea
